...
test("CPU entries preserve rivals on retry and update separate adaptive records exactly once",async()=>{
 ...
 const call=async(path:string,method="GET",body?:unknown)=>{
  ...
  const result=r.status===204?null:await r.json();assert.ok(r.ok,JSON.stringify(result));return result;
 };
 token=(await call("/auth/guest","POST",{adultConfirmed:true})).token;
 try {
  const request={venueId:"heritage",requestId:randomUUID(),mode:"cpu"};
  ...
  await Promise.all([1,2].map(()=>call(`/local-matches/${request.requestId}/finish`,"POST",{outcome:"lost"})));
  let p=await call("/me");
  assert.equal(p.stats.cpuLosses,1);assert.equal(p.stats.cpuStreak,-1);assert.equal(p.stats.localLosses,undefined);
  const second=await call("/local-matches/start","POST",{...request,requestId:randomUUID()});
  assert.ok(second.match.opponent.skill<skill);
  await call(`/local-matches/${second.match.id}/finish`,"POST",{outcome:"won"});
  p=await call("/me");assert.equal(p.stats.cpuStreak,1);assert.equal(p.stats.cpuWins,1);assert.equal(p.coins,900);
 } finally {await call("/me","DELETE",{confirm:"DELETE"});}
});
